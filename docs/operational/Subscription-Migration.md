...
Some parts of this, for example `elz_environment` and `environement_prefix` are assigned via GitHub Actions (as seen below)
...
1. Open the Azure Portal and verify the environment custom domain name via the Front Door profile
2. Open a *Service Now* request to validate and approve the domain name
    1. You may need to provide the DNS verification record determined in step 1 above
    2. You will need to provide the endpoint association address from Front Door for the CNAME record
3. Once the domain has been verified and the DNS has propagated, you may need to wait up to 24 hours for the DNS to take effect
4. Once Application Insights and Front Door logs show no further traffic is hitting the old subscription, you can stop the App Service
5. Once the environment has been fully migrated, you can remove the following resource groups for each environment in the old subscription:
    - translator-rg
    - redis-rg
    - core-rg
    - load-testing
