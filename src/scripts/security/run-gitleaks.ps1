...
function Get-CacheRoot {
    if ($IsWindows) {
        return Join-Path $env:LOCALAPPDATA "care-leavers/tools"
    }

    if ($env:XDG_CACHE_HOME) {
        return Join-Path $env:XDG_CACHE_HOME "care-leavers/tools"
    }

    return Join-Path $HOME ".cache/care-leavers/tools"
}
...
if ($env:CL_SKIP_GITLEAKS -eq "1") {
    Write-Host "Skipping GitLeaks because CL_SKIP_GITLEAKS=1."
    exit 0
}
...
try {
    ...
    Write-Host "For urgent commits only, rerun with CL_SKIP_GITLEAKS=1."
    exit $exitCode
}
finally {
    Pop-Location
