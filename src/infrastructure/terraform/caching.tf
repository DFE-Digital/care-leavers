...
resource "azurerm_key_vault_secret" "redis-enterprise-connection-string" {
  ...
  value        = "${azurerm_managed_redis.redis-enterprise.hostname}:${azurerm_managed_redis.redis-enterprise.default_database[0].port},ssl=true,password=${azurerm_managed_redis.redis-enterprise.default_database[0].primary_access_key}"
}